...
<head>
<meta charset="UTF-8">
<title>search shopkeeper items</title>
<style type="text/css">
.title {
...
<body>
	...
	<div class="grid_container">
		...
		<div
			...
			<table class="table_content">
				...
				String search_item = request.getParameter("search_box_items");
				
				if ( search_item.equals("") || search_item == null || search_item == "" ) { 
					request.setAttribute("shopk_search_item_status", "invalid_search");
				...
				String select_query_for_table = "SELECT * FROM products_table WHERE item_name LIKE '%" + search_item + "%';";
				...
				try {
					...
					Statement select_st_prod_table = con.createStatement();
					
					ResultSet res_set = select_st_prod_table.executeQuery(select_query_for_table);

					if (!res_set.next()) {
						request.setAttribute("shopk_search_item_status", "no_item_exists");
						req_Dispatcher = request.getRequestDispatcher("shopkeeper_items.jsp");
						req_Dispatcher.forward(request, response);
					} else { 
	...
	<script type="text/javascript">
		...
		var qua_value = document.getElementById("items").value;
		...
		if ( cart_status_code == "i_cart_qua" ) {
			swal("congratulations", "The" + qua_value + " number of products was added to the cart successfully", "success");
		} 
