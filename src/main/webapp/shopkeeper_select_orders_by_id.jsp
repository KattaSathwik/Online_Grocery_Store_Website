...
<head>
<meta charset="UTF-8">
<title>Shopkeeper seen orders items by search id</title>
<style type="text/css">
.title {
...
<body>
	...
	<div class="grid_container">
		<div class="cell" id="container_title">
			<h1 class="title">These products are ordered by the customers : </h1>
		</div>

		<h1 class="heading"> Ordered Items </h1>
		...
		<div
			...
			<table class="table_content">
				...
				<tr>
					...
					<th><a class="text_link" href="#">Orderd_date</a></th>
				</tr>
				...
				String str_search_cust_id = request.getParameter("cust_id");
				
				if ( str_search_cust_id == "" || str_search_cust_id.equals("") || str_search_cust_id == null ) { 
					request.setAttribute("shopk_id_order_item_status", "cust_invalid_id");
					req_Dispatcher = request.getRequestDispatcher("shopkeeper_order_cond.jsp");
					req_Dispatcher.forward(request, response);
				}
				
				int search_cust_id = Integer.parseInt(request.getParameter("cust_id"));
				...
				String select_query_for_table = "SELECT * FROM orders_table WHERE customer_id = ?";
				...
				try {
					...
					/*if ( res_set_cust.next() ) {
						...
						request.setAttribute("cust_order_item_status", "user_not_exist");
						req_Dispatcher = request.getRequestDispatcher("person_profile.jsp");
						req_Dispatcher.forward(request, response);
					}*/
					
					PreparedStatement select_prepst_order_table = con.prepareStatement(select_query_for_table);
					
					select_prepst_order_table.setInt(1, search_cust_id);

					ResultSet res_set = select_prepst_order_table.executeQuery();

					if (!res_set.next()) { 
						request.setAttribute("shopk_id_order_item_status", "no_item_exists");
						req_Dispatcher = request.getRequestDispatcher("shopkeeper_order_cond.jsp");
						req_Dispatcher.forward(request, response);
					} else { 
						//row_count = prepst_emp_table.executeUpdate(); 
						order_id = res_set.getInt(1);
						...
						ordered_date = res_set.getString(9);
					...
					<td><b style="color: brown;"> <%=ordered_date%>
					</b></td>
				</tr>
			...
			<h1 class="heading"> Total Price customer will get : <%= item_total_price_cnt %> </h1>
	...
	<script type="text/javascript">
		...
		if ( order_status_code == "no_item_exists") {
			swal("Sorry", "No items we exists in the order table", "error");
		} 
